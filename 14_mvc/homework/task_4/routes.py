...
from models import  get_the_most_popular_book
...
"""
    rows: str = ''
    for book in books:
        rows += '<tr><td>{id}</tb><td>{title}</tb><td>{author}</tb><td>{views_counter}</tb></tr>'.format(
            id=book['id'], title=book['title'], author=book['author'],
            views_counter=book['views_counter']
        )
...
@app.route('/books/id')
def the_most_popular_book() -> str:
    return render_template('index.html', books=get_the_most_popular_book())
